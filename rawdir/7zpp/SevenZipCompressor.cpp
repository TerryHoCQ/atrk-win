...
bool SevenZipCompressor::SetCompressionProperties(IUnknown* outArchive) const
{
	...
	CPropVariant values[kNumProps] =
	{
	  m_encrypt_header,    // solid mode OFF
	  static_cast<UInt32>(m_compressionLevel.GetValue())
	};
	...
	HRESULT hr = setter->SetProperties( names, values, kNumProps);
